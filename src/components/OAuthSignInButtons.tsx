...
export function OAuthSignInButton({
  ...
  const onClick = useCallback(async () => {
    setBusy(true)
    try {
      await signInWithDiaryOAuth(supabase, provider)
    } catch (err) {
      const raw =
        err && typeof err === 'object' && 'message' in err && typeof (err as Error).message === 'string'
          ...
          : ''
      const msg =
        raw || `Impossibile aprire ${providerName}. Controlla la connessione o riprova tra poco.`
      onError?.(msg)
      setBusy(false)
    }
  }, [supabase, provider, providerName, onError])
